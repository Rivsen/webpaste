    ...
    public function loadTemplate ($tmplTemplateFile,$prefix='') {
        ...
        if(C('LAYOUT_ON')) {
            ...
            // 替换布局的主体内容
                ...
                $layoutFile = str_replace($this->config['layout_item'],'{$__CONTENT__}',file_get_contents($layoutFile));
                $tmplContent = '<?php ob_start();?>'.$tmplContent.'<?php $__CONTENT__=ob_get_clean();?>'.$layoutFile;
            }
        }
        // 编译模板内容
        ...
        if(!is_dir($dir))
            mkdir($dir,0755,true);
        //重写Cache文件
        if( false === file_put_contents($tmplCacheFile,str_replace('{__NOLAYOUT__}','',trim($tmplContent))))
    ...
    protected function parseLayout($content) {
        ...
        if($find) {
            ...
            if(!C('LAYOUT_ON') || C('LAYOUT_NAME') !=$array['name'] ) {
				...
				$layoutFile = str_replace($replace,'{$__CONTENT__}',file_get_contents($layoutFile));
				$content = '<?php ob_start();?>'.$content.'<?php $__CONTENT__=ob_get_clean();?>'.$layoutFile;
            }
        }
        return $content;
    }
