...
class TagLibHtml extends TagLib{
    ...
    protected $tags   =  array(
        ...
        'select'    => array('attr'=>'name,options,values,output,multiple,id,size,first,selected,style,class','close'=>0),
        'grid'      => array('attr'=>'id,pk,class,style,action,actionlist,show,datasource','close'=>0),
        'list'      => array('attr'=>'id,pk,class,style,action,actionlist,show,datasource,checkbox,footer,noresult','close'=>0),
        'imagebtn'  => array('attr'=>'id,name,value,type,style,click','close'=>0),
        'checkbox'  => array('attr'=>'name,options,checked,separator','close'=>0),
    ...
    public function _select($attr) {
        ...
        $parseStr = '<select id="'.$id.'" name="'.$name.'"'.$multiple.' class="'.$cls.'" >';
        if(!empty($first)) {
    ...
    public function _checkbox($attr) {
        ...
        if(!empty($options)) {
        	$parseStr   .= '<?php  foreach($'.$options.' as $key=>$val) { ?>';
        	if(!empty($checked)) {
        		$parseStr   .= '<label><input type="checkbox"<?php if(isset($'.$checked.') && ($'.$checked.' == $key || in_array($key,$'.$checked.'))) { ?> checked="checked"<?php } ?> name="'.$name.'[]" value="<?php echo $key ?>" /><?php echo $val ?>'.$separator.'</label>';
        	}else {
        		$parseStr   .= '<label><input type="checkbox" name="'.$name.'[]" value="<?php echo $key ?>" /><?php echo $val ?>'.$separator.'</label>';
        	}
        	$parseStr   .= '<?php } ?>';
        }
        return $parseStr;
    ...
    public function _grid($attr) {
        ...
        $parseStr  .= '<table id="'.$id.'" class="'.$cls.'" style="'.$style.'">';
        $parseStr  .= '<tr class="tableHeadFill"><td colspan="'.$colNum.'"></td></tr>';
        ...
        $parseStr	.= '</tr></volist><tr class="tableFootFill"><td colspan="'.$colNum.'"></td></tr></table>';
        $parseStr	.= "\n<!-- Think 系统列表组件结束 -->\n";
        return $parseStr;
    }
    ...
    public function _list($attr) {
        ...
        //显示开始
		$parseStr	= "<!-- List Start -->\n";
        $parseStr  .= "<table id='{$id}' class='{$cls}'>";
        $parseStr  .= '<thead><tr>';
        //列表需要显示的字段
        $fields = array();
        foreach($show as $val) {
        ...
        if(!empty($checkbox) && 'true'==strtolower($checkbox)) {//如果指定需要显示checkbox列
            $parseStr .="<th width='8'><input data-action='checkall' value='{$id}' type='checkbox'></th>";
        }
        if(!empty($key)) {
            $parseStr .= '<th width="12">No</th>';
        }
        foreach($fields as $field) {//显示指定的字段
            ...
            if($sort) {
                $parseStr .= '<a class="sorter" href="#'.$property[0].'/{$sort}" data-action="sort" title="按照'.$showname[2].'{$sortType} ">'.$showname[0].'<eq name="order" value="'.$property[0].'" ><span>&<?php echo $sort == 1 ? "d" : "u"?>Arr;</span></eq></a></th>';
            }else{
                $parseStr .= $showname[0].'</th>';
            }
            ...
            if(!empty($field[2])) {
                ...
                if(count($href)>1) {
                    ...
                    if(count($array)>1) {
                        ...
                        $parseStr .= '<a data-action="js" href="javascript:'.$href[0].'('.implode(',',$temp).')">';
                    }else{
                        $parseStr .= '<a data-action="js" href="'.$href[0].'/{$'.$name.'.'.$href[1].'}">';
                    }
                }else {
                    //如果没有指定默认传编号值
                    $parseStr .= '<a href="javascript:'.$field[2].'(\'{$'.$name.'.'.$pk.'|addslashes}\')">';
            ...
            if(!empty($actionlist[0])) {//显示指定的功能项
                ...
                foreach($actionlist as $val) {
					if(strpos($val,':')) {
						$a = explode(':',$val);
						if(count($a)>2) {
                            $parseStr .= '<a data-action="js" href="#'.$a[0].'/{$'.$name.'.'.$a[2].'}">'.$a[1].'</a>&nbsp;';
						}else {
							$parseStr .= '<a data-action="js" href="'.$a[0].'/{$'.$name.'.'.$pk.'}">'.$a[1].'</a>&nbsp;';
						}
					}else{
						$array	=	explode('|',$val);
						if(count($array)>2) {
        ...
        $parseStr	.= '</tr></volist><if condition="empty($'.$datasource.')"><tr><td colspan="'.$colNum.'">'.$noresult.'</td></tr></if></tbody><tfoot><tr><td colspan="'.$colNum.'">'.(empty($footer) ? "" : "{\$$footer}").'</td></tr></tfoot></table>';
        $parseStr	.= "\n<!-- List End -->\n";
        return $parseStr;
    }
}