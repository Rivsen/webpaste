...
class RelationModel extends Model {
                            ...
                            switch($mappingType) {
                                case HAS_ONE:
                                    switch (strtoupper($opType)){
                                        ...
                                        break;
                                        case 'SAVE':    // 更新关联数据
                                        $result   =  $model->where($mappingCondition)->update($mappingData);
                                        if(!$result) {
                                        	//try insert
                                        	$mappingData[$mappingFk]    =   $pk;
                                        	$result   =  $model->add($mappingData);
                                    ...
                                    switch (strtoupper($opType)){
                                        ...
                                        foreach ($mappingData as $vo){
                                            if(isset($vo[$pk])) {// 更新数据
                                                ...
                                                $result   =  $model->where($mappingCondition)->update($vo);
                                            }else{ // 新增数据
                                ...
                                case MANY_TO_MANY:
                                    ...
                                    $mappingRelationTable  =  $val['relation_table']?$val['relation_table']:$this->getRelationTableName($model);
                                    if(is_array($mappingData) && !empty($mappingData)) {
                                    	if(isset($mappingData[0][$mappingKey])) {
	                                        $ids   = array();
	                                        foreach ($mappingData as $vo)
	                                            $ids[]   =   $vo[$mappingKey];
	                                        $relationId =   implode(',',$ids);
                                    	}else {
                                    		//一些时候设置值是直接为键的值数组，省略了键名
                                    		$relationId = implode(',',$mappingData);
                                    	}
                                    }
                                    switch (strtoupper($opType)){
                                        case 'ADD': // 增加关联数据
    ...
    public function link($name, $key, $value){
    	if(is_array($name)) {
    		array_merge($this->_link, $name);
    	}else if(is_array($key)){
    		array_merge($this->_link[$name], $key);
    	}else{
    		$this->_link[$name][$key] = $value;
    	}
    	return $this;
    }
}